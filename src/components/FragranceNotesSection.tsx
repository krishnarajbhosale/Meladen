...
export default function FragranceNotesSection({ notes, custom = 7 }: Props) {
  ...
  return (
    ...
    >
      <div className="mb-3">
        <p className="mb-0.5 text-[10px] uppercase tracking-[0.22em] text-brand-gray">Scent profile</p>
      ...
      <div className="space-y-3">
        {layers.map(layer => (
          <div key={layer.key} className="flex flex-col gap-1.5 sm:flex-row sm:items-baseline sm:gap-3">
            <p className="shrink-0 text-[10px] font-medium uppercase tracking-[0.18em] text-brand-sage sm:w-24">
              {layer.label}
            </p>
